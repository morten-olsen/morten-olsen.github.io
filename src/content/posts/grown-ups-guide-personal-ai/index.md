...
❄️ **Weather:** 4°C and overcast. Heavy rain expected around 3 PM.

🚆 **Trains:** Partial closure on Line B. Replacement buses running between Central Station and The Office due to technical difficulties. Next update expected at 7:30.

📰 **News:** Government announces new green energy initiative; Local zoo welcomes baby giraffe.
...
1. **Surprise Scoring**: Right now, GLaDOS logs *everything*. But our brains don't. We only remember the "surprising" things—the events that broke our prediction model (Google's Titans architecture uses a similar concept). I need to implement a filter that discards the predictable routine and only keeps the anomalies.
